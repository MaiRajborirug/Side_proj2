...
def c_factor(n):  # cy_usporf.c_factor()
    """
    ...
    return 2.0*(np.log(n-1)+Euler_const) - (2.0*(n-1.)/(n*1.0))
...
def FastBIC(Z_arr):  # USPORF's algo 3
    ...
    for s in range(2, (N+1)-2):
        Curr_1 = Z_arr[Z_ind[:s]]  # left-daughter
        Curr_2 = Z_arr[Z_ind[s:]]  # Right-daughter
        var_1 = np.var(Curr_1)
        var_2 = np.var(Curr_2)
